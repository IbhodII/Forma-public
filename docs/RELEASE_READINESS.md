...
## Public Repository Data Policy (GitHub)

Before publishing **Forma-Public**:

| File | GitHub |
|------|--------|
| Root `shared.db` | **Yes** — only after `build_public_shared_db.py` + `audit_public_shared_db.py` → **READY FOR GITHUB** |
| `workouts.db` | **Never** — personal workouts, meal plans, tokens, sync state |
| `packaging/seed/*.db` | Generated at build; gitignored |
| Dev `shared.db` (unsanitized) | **Never** — may contain meal tables, OFF cache, bloated lookup duplicates |

Canonical policy: [DATABASE.md § Public Repository Data Policy](./DATABASE.md#public-repository-data-policy).
...
| P1 | Goal deficit validation rejects >60 kcal/kg fat | Fixed | [KNOWN_ISSUES.md](./KNOWN_ISSUES.md), [NUTRITION.md](./NUTRITION.md) |
...
- Route point telemetry **phase 1** (per-point popup fields) shipped 2026-06-09; **phase 2** (map↔chart sync) is P2, not a release blocker — [ROADMAP.md](./ROADMAP.md).
- Dev/Public migration numbering mismatch is **not** a Public installer smoke blocker. Public remains authoritative for Public builds at `SCHEMA_VERSION=80`; Dev reconciliation should append a future migration rather than renumbering history.
...
1. `cd frontend && npm run desktop:dist` (или готовый installer; включает `desktop:prepare-seed` + `desktop:check-secrets`).
2. Clean install seed: `packaging/seed/workouts.db` and `packaging/seed/shared.db` exist, are below the size cap, and were generated by `desktop:prepare-seed` (not copied from dev by hand).
3. Public shared DB: run `build_public_shared_db.py` + `audit_public_shared_db.py`; root `shared.db` and `packaging/seed/shared.db` must report **READY FOR GITHUB**.
4. Clean install OAuth: Google/Yandex PKCE with public template only — [AUTH_PKCE_AUDIT.md](./AUTH_PKCE_AUDIT.md); Polar 503 until user adds `POLAR_CLIENT_SECRET` — expected ([POLAR_SETUP.md](./POLAR_SETUP.md)).
5. Первый запуск: без экрана admin login; сессия `user_id=1`.
6. Settings → Данные → Резервные копии → **Создать резервную копию** → ZIP на диск.
7. **Восстановить из резервной копии** → тот же ZIP, Replace → job done.
8. Restart app → `GET /api/health` 200 (default port **8000**).
9. Убедиться: нет mini DB / JSON / 2-file в UI.
10. `/workouts`: создать обычную тренировку; проверить prefill веса/повторов/разминки из истории.
11. `/workouts`: создать/открыть шаблон с суперсетом или кругом; проверить компактный блок, раскрытие и редактирование текущих подходов без входа в структуру.
12. `/workouts` → «Набор упражнений»: проверить простой список, структуру блоков и autocomplete упражнений.
13. Каталог упражнений: edit/delete мусорной записи; used exercise должен архивироваться, история не должна меняться.
14. `/food`: открыть неделю, день, продукт/составное блюдо, meal plan apply smoke.
15. `/body`: open measurements chart/history. Do **not** sign off until edit action from chart/history no longer crashes.
16. `/workouts`: create a new exercise template/set with normal + superset/circuit blocks and verify names/order/exercise assignments are preserved.
...
11. `npm run desktop:check-secrets` green (also enforced by `desktop:dist`).
12. Seed databases present under `packaging/seed/` and under 100 MB — [PACKAGING_SECRETS.md](./PACKAGING_SECRETS.md).
13. Packaged API health on default port **8000** (or persisted port in `forma-desktop-api.json`).
14. Documentation synchronized if release touched migrations, OAuth, packaging, DB ownership, sync, mobile modes or units.
15. If Dev migration reconciliation is later implemented, verify future shared migration numbering starts from v081+ only after Dev reaches its appended reconciliation version.
...
| Packaging secret check | | |
| Clean install OAuth (PKCE) | | |
| Polar setup doc (user secret) | | |
